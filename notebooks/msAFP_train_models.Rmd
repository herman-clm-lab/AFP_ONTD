...
model_dict_qr <- list(m1_gw_q = model_1_gw_q,
...
predictions <- map(names(model_dict),
                   \(m) {
                    train_df %>%
                        unnest(!!m) %>%
                        mutate(model = !!m) %>%
                        select(EMPI, ACCESSION_NUM, ORDER_ID, DRAWN_DT_TM, race_factor, `Insulin Depend. Diabetic`, model, AFP, logAFP, .AFP_pred, .AFP_MoM, .AFP_MoM_LL, .AFP_MoM_UL, .NTD_screen)
                    })  |> list_rbind()
```
...
#### Overlap between CI's of m1_gw_q and m2_gwr_q AFP_MoMs
...
fig2a <- tmp %>%
  ...
  scale_x_continuous(name = "msAFP-r MoM", limits=c(0, 6)) +
  scale_y_continuous(name = "msAFP MoM", limits = c(0, 6)) +
...
ggsave("../output/Figure2.svg", plot = fig2, width = 8, height = 5, u.Pnits = "in")
